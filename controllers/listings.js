...
module.exports.index = async (req, res) => {
    ...
    if (search) {
      ...
      allListings = await Listing.find({ $and: regexConditions });
    } else if (category) {
...
module.exports.showListing = async (req, res) => {
  ...
  // Separate reservations for owner and current user
  const allReservations = listing.reservations || [];
  const userReservations = allReservations.filter(r => 
      req.user && r.user._id.equals(req.user._id)
  );
